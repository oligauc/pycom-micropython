...
static void alljoin_validate_mode (uint mode);
static void initialise_aj_args(mp_obj_t *arguments, mp_uint_t number_arguments);
...
STATIC mp_obj_t alljoyn_make_new(const mp_obj_type_t *type, mp_uint_t n_args, mp_uint_t n_kw, const mp_obj_t *all_args) {
    ...
    alljoin_validate_mode (self->mode);
...
STATIC mp_obj_t add_interface(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    for (uint8_t idx = 0; idx < number_methods; idx++){
        ...
        if (alljoyn_obj.mode == AJ_SERVICE){
            ...
            alljoyn_obj.callbacks[idx] = callbacks[idx];
        } else {
            addObject((char *)service_path, (char *)interface_name, &method_name, 1, MP_OBJ_NULL);
        }
    }
...
STATIC mp_obj_t call_method(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    /*if (!wlan_is_connected()){
        mp_raise_ValueError("No network available\n");
    }*/
    ...
    printf("SP: %s, IN: %s, IM: %s",service_path, interface_name, interface_method);
    ...
    if ((number_arguments % 2) != 0){
        mp_raise_ValueError("Method arguments format: (Type1, Arg1, Type2, Arg2, etc)\n");
    }
    initialise_aj_args(arguments, number_arguments);
...
STATIC const mp_map_elem_t alljoyn_locals_dict_table[] = {
   ...
   { MP_OBJ_NEW_QSTR(MP_QSTR_STRING),                 MP_OBJ_NEW_SMALL_INT(AJ_STRING)},
};
...
static void initialise_aj_args(mp_obj_t *arguments, mp_uint_t number_arguments) {
    ...
    alljoyn_obj.aj_args = (AJ_Arg *)malloc(number_arguments * sizeof(AJ_Arg));
